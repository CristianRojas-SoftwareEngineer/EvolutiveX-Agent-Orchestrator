...
**Configuración operativa:** Claude Code hooks apuntan al proxy vía `.claude/settings.json` del proyecto. El archivo del proyecto registra **13 claves en total**: las **8 del lifecycle** que alimentan al gateway (`UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `SubagentStart`, `SubagentStop`, `Stop`, `StopFailure`) y **5 claves de UX no-lifecycle** que solo emiten notificación de escritorio (`SessionStart` con matcher `startup|resume`, `SessionEnd`, `PermissionRequest`, `TaskCreated`, `TaskCompleted`). Las 13 sobrescriben las entradas equivalentes del user-level `C:\Users\Cristian\.claude\settings.json` (merge por clave: el proyecto tiene precedencia). Los relays de lifecycle leen stdin **una vez** (UTF-8) y hacen `POST` a `$ANTHROPIC_BASE_URL/hooks` vía `fetch` ([`post-hook-event.ts`](../scripting/post-hook-event.ts), [`gateway-hook-notify.ts`](../scripting/gateway-hook-notify.ts), [`pre-tool-use-hook-ux.ts`](../scripting/pre-tool-use-hook-ux.ts), [`stop-hook-ux.ts`](../scripting/stop-hook-ux.ts)). `PreToolUse` y `PostToolUse` usan `matcher: "*"`. **`UserPromptSubmit`** y **`StopFailure`** usan `gateway-hook-notify` (gateway + toast sin competir por stdin). **`PreToolUse`** usa `pre-tool-use-hook-ux` (gateway + toast condicional en `AskUserQuestion`). **`Stop`** usa `stop-hook-ux` (gateway + toast de continuidad). **`SubagentStart`** / **`SubagentStop`** combinan `post-hook-event` con CLI de mensaje fijo. **`PostToolUse`** / **`PostToolUseFailure`**: solo `POST /hooks`. Las **5 claves de UX no invocan `POST /hooks`** (el `AuditHookEventHandler` solo despacha los 8 del lifecycle). Tabla completa: [`docs/notifications.md`](notifications.md).
