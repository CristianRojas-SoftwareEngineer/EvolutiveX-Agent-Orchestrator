...
> **Nota sobre la divergencia histórica del provider de texto:** el código actual (`AuditHookEventHandler.generateSpeechText`) usa `gemini-2.5-flash` (`GEMINI_FLASH_URL`) para componer el texto, no OpenRouter. Esta divergencia es **anterior** a la migración a sidecar local (`c00076`) y queda fuera del alcance de ese cambio. Cuando se introduzca el fallback OpenRouter para texto, se alineará el código con la doc.
...
| -------------------- | ------- | -------------------------------------------------------------------------------------------------------- |
| `UserPromptSubmit`   | —       | `npx tsx scripting/hooks/post-hook-event.ts` (gateway emite toast con preview del `prompt`)              |
| `PreToolUse`         | `*`     | `npx tsx scripting/hooks/post-hook-event.ts` (gateway decide toast condicional `AskUserQuestion`)        |
| `PostToolUse`        | `*`     | `npx tsx scripting/hooks/post-hook-event.ts` (gateway decide toast condicional `TaskUpdate+in_progress`) |
