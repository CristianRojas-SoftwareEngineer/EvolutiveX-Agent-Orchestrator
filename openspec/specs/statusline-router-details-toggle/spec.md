...
`SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS`.

## Requirements

### Requirement: Activar visibilidad de Tabla 2

La CLI SHALL escribir `on` en `settings.env.SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS`
al ejecutar el subcomando `on`, preservando todas las demás claves del bloque `env`.

#### Scenario: Subcomando on — variable ausente

- **WHEN** `settings.env` no contiene `SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS`
- **AND** se ejecuta `statusline-router-details.ts on`
- **THEN** el archivo `settings.json` SHALL contener `"SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS": "on"`
- **AND** las demás claves del bloque `env` SHALL permanecer sin cambios

#### Scenario: Subcomando on — variable ya en on

- **WHEN** `settings.env.SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS` es `"on"`
- **AND** se ejecuta `statusline-router-details.ts on`
- **THEN** el archivo SHALL seguir conteniendo `"SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS": "on"`

#### Scenario: Subcomando on — variable en off

- **WHEN** `settings.env.SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS` es `"off"`
- **AND** se ejecuta `statusline-router-details.ts on`
- **THEN** el archivo SHALL contener `"SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS": "on"`

### Requirement: Desactivar visibilidad de Tabla 2

La CLI SHALL escribir `off` en `settings.env.SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS`
...
- **THEN** el archivo SHALL contener `"SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS": "off"`
...
La CLI SHALL invertir el valor de `SMART_CODE_PROXY__STATUSLINE_ROUTER_DETAILS` al
...
todas las demás claves de `env` (e.g. `SMART_CODE_PROXY_ROOT`, `ANTHROPIC_*`) y
del resto de `settings.json` SHALL conservarse intactas.

#### Scenario: Escritura con claves preexistentes

- **WHEN** `settings.env` contiene `SMART_CODE_PROXY_ROOT` y otras claves
