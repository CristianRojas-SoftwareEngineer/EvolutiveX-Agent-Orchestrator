# statusline-live-refresh Specification

## Purpose

Actualización visible per-step de la Tabla 2 del statusline oficial de Claude Code mediante `refreshInterval`, cierre temprano con caché de `mtime`/`size`, e indicador visual del modo live.
...
Cuando `router-status` se invoca y `session-metrics.json` existe en la sesión, SHALL comparar el `mtime` Y el `size` del archivo con los valores cacheados en `.statusline-state.json`. Si el `mtime` y `size` coinciden con el cache y `lastRenderedTable2Output` está presente, SHALL imprimir el último render cacheado desde `.statusline-state.json` y SHALL NOT re-renderizar la Tabla 2 desde cero.
