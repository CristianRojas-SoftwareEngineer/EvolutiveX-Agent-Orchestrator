...
describe('isSmartCodeStatusLine', () => {
  it('retorna true si el comando incluye router-status.ts', () => {
    expect(isSmartCodeStatusLine('npx tsx /repo/scripting/provider/router-status.ts')).toBe(true);
  });

  it('retorna true con backslashes Windows', () => {
    expect(isSmartCodeStatusLine('C:\\repo\\scripting\\provider\\router-status.ts')).toBe(true);
  });
...
describe('buildStatusLineCommand', () => {
  it('genera el comando npx tsx para el proxyRoot dado', () => {
    ...
    expect(cmd.replace(/\\/g, '/')).toContain('scripting/provider/router-status.ts');
...
describe('shouldOverwriteStatusLine', () => {
  ...
  it('ok si el statusLine existente es de SCP', () => {
    expect(
      shouldOverwriteStatusLine('npx tsx /repo/scripting/provider/router-status.ts', false),
    ).toEqual({
      ok: true,
    });
  });
...
describe('applyStatuslineUninstall', () => {
  it('borra statusLine de SCP sin --force', () => {
    const settings: ClaudeSettings = {
      statusLine: { command: 'npx tsx /repo/scripting/provider/router-status.ts' },
