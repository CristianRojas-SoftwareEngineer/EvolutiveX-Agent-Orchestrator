# Spec: hooks-observability-hardening

## Purpose

Define el endurecimiento de observabilidad del borde hooks: niveles de log correctos para eventos de cierre no correlacionados, detección temprana de payloads malformados en el controlador HTTP, exit codes semánticos en el relay, y cobertura completa de `SessionStart`. Implementado en capa 3 (`AuditHookEventHandler`), capa 5 (`HooksController`), scripting (`post-hook-event.ts`) y config (`configs/hooks.json`).
