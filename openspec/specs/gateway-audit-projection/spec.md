...
`delegateClosure()` en `AuditHookEventHandler` SHALL, cuando el workflow sea de kind `main`, invocar `sessionMetrics.finalizeWorkflowMetrics()` (incremento de `workflow_count` / reconciliación de workflow) y NO SHALL re-ejecutar un merge completo de steps/tokens que duplique hops ya contabilizados per-step en el mismo workflow. NO SHALL invocar `AuditWorkflowClosureHandler.execute()` ni resolver rutas flat legacy.
...
- **THEN** SHALL invocar `sessionMetrics.finalizeWorkflowMetrics()` para ese workflow
