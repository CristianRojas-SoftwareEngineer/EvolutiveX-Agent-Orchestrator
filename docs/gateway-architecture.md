...
**Configuración operativa:** Claude Code hooks apuntan al proxy vía `.claude/settings.json` del proyecto. El archivo del proyecto registra **14 entradas en total**: las **8 entradas del lifecycle** que alimentan al gateway (`UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `SubagentStart`, `SubagentStop`, `Stop`, `StopFailure`) y **6 entradas adicionales de UX no-lifecycle** que solo emiten notificación de escritorio (`SessionStart` con matcher `startup|resume`, `SessionEnd`, `PermissionRequest`, una segunda entrada `PreToolUse` con matcher `AskUserQuestion`, `TaskCreated` y `TaskCompleted`). Las 14 sobrescriben las entradas equivalentes del user-level `C:\Users\Cristian\.claude\settings.json` para esas claves (mecanismo de merge de Claude Code: el proyecto tiene precedencia por clave). Las 8 entradas del lifecycle contienen un comando `npx tsx scripting/post-hook-event.ts` (relay en [`scripting/post-hook-event.ts`](../scripting/post-hook-event.ts): lee stdin y hace `POST` a `$ANTHROPIC_BASE_URL/hooks` vía `fetch`; compatible con PowerShell, macOS y Linux). Los matchers de `PreToolUse` y `PostToolUse` para `POST /hooks` se establecen en `*` para que el gateway reciba los eventos de todas las tools. Los hooks **`UserPromptSubmit`**, **`SubagentStart`**, **`SubagentStop`** y **`StopFailure`** llevan además un segundo comando al CLI de notificaciones (`src/2-services/notifications/cli.ts`). El hook **`Stop`** en este repositorio usa un relay unificado [`scripting/stop-hook-ux.ts`](../scripting/stop-hook-ux.ts): un proceso lee stdin, hace `POST /hooks` y emite dos toasts (fin de turno + resumen con modelo); ver [`docs/notifications.md` § Hook Stop](notifications.md#hook-stop-fin-de-turno-y-resumen-con-modelo). Los 3 hooks restantes del lifecycle (`PreToolUse` con `*`, `PostToolUse` con `*`, `PostToolUseFailure`) llevan únicamente el comando `POST /hooks`. **Las 6 entradas de UX no invocan `POST /hooks`**: el `AuditHookEventHandler` solo despacha los 8 `eventName` del lifecycle (el resto cae en `default:` y se descarta), así que `POST /hooks` desde esas claves sería tráfico desperdiciado. El reapuntamiento del 2º comando desde el script externo `C:\AI\claude-code-notifications.ts` hacia el entry point del repositorio se cubrió en la fase N2 del roadmap `claude-code-hooks-implementation`; el script externo queda marcado como `@deprecated` con fecha de retirada prevista 2026-09-01. Detalle operativo del servicio de notificaciones, tabla completa de 14 entradas y nota de override: [`docs/notifications.md`](notifications.md).
