...
status: done
verdict: resuelto — fallback tool_result en continuation + finalize métricas wire; reinstalar hooks PostToolUse pendiente operacional
---

# Case Manifest — 20260608-proxy-audit-residual-gaps

## Case

Brechas residuales en la auditoría causal del Smart Code Proxy detectadas al validar el fix del caso `20260608-proxy-audit-discrepancies` contra la sesión `c5eb2667-8de6-42ab-8474-ba82877b344c`: `tool_result` no persistido (tools en `running`), `session-metrics.json` con `total_workflows: 0` pese a workflows en disco, `stepCount: 0` en el workflow sesión, y ruido de hooks desconocidos en `server/logs.jsonl`.

## Profile parameters

- **Priorities:** reproduce → root cause → minimal fix → no regression.
- **Success metrics:** test red→green; `tool_result` en `events.ndjson` y `result.json` en disco; `total_workflows` coherente con workflows wire cerrados; cero regresiones.
...
openspec_change: "fix-proxy-tool-result-metrics"
case_run: 1
case_paused_at: ""
case_resumed_at: ""
