...
4. **Design decisions — mandatory pre-drafting clarification gate**: after completing discovery and **before drafting any plan section**, compile every unresolved decision point, ambiguity, competing strategy, and missing requirement surfaced by the requirements text and the discovered codebase. Surface them to the user as a batch via the harness's structured-question capability (Claude Code: `AskUserQuestion`): one question per decision, each alternative as a selectable option, the option description must state explicit trade-offs (pros and cons), options ordered from most to least recommended with the top option marked as the recommendation. Do not draft any section until the user has responded to every open question. If requirements are incomplete, add a free-text question rather than guessing. If a new decision point surfaces mid-draft that was not visible at discovery time, apply the same pattern: stop, present it as a structured question, resume drafting only after the user decides. Do not resolve any decision unilaterally. When competing strategies exist and a maintenance profile is active (declared by the user or received from an invoking flow), weight the presented options by profile: **correctivo** → diff size, reversibility, and non-regression; **perfectivo** → dominant metric and significance; **preventivo** → coverage of risk-materialization paths and residual risk; **adaptativo** → reversibility, feature-flag isolation, and contract preservation.
