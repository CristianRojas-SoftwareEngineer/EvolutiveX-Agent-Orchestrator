...
Cada **petición HTTP** que el proxy reenvía a `api.anthropic.com` genera un directorio de auditoría:

```text
sessions/<session-id>/requests/NNNNNN_<requestId>/
```

- `NNNNNN` es el orden dentro de la sesión (`requestSequence` en `meta.json`; debe ser coherente con el prefijo del directorio y con `request-sequence.json` en la raíz de la sesión).
- `meta.json` describe método, URL, si la respuesta es SSE (`sse`), bytes, etc.

Eso es una **interacción de red** única: una fila en el historial de auditoría.
...
**Petición de referencia:** `sessions/claude-code-workflow-example/requests/000006_5e0986b8-ff70-4afc-9534-701bb9c68597/response.body.formatted.json` (modelo `claude-haiku-4-5-20251001`).
...
Para localizar `usage` en disco (jerarquía general: `sessions/<session-id>/requests/NNNNNN_<requestId>/`):

| `meta.json` (`sse`) | Archivos donde aparece el mensaje / `usage`                                                               |
| ------------------- | --------------------------------------------------------------------------------------------------------- |
| `false`             | `response.body.json`, `response.body.formatted.json`, opcionalmente `response.body.parsed.md`.            |
| `true`              | Siempre `response.sse.jsonl` (stream de eventos). Con `AUDIT_SSE_RAW=1` puede existir `response.sse.txt`. |
...
1. Recorre cada carpeta `requests/NNNNNN_*/` en orden de secuencia (p. ej. orden numérico del prefijo o `requestSequence` en `meta.json`).
...
3. Para cada `POST` a `/v1/messages` con respuesta válida y `usage` disponible, aplica la §8. El `model` suele estar en el cuerpo de respuesta reconstruido o en el último evento SSE del stream.
4. **Suma** los costes por petición. Si el proxy registró error de upstream (cuando exista un campo equivalente en `meta`), si `responseReceived` es falso, o si no hay archivos de respuesta utilizables, no hay `usage` fiable para esta ecuación.
