import { JsonValue, JsonObject } from '../types/json.types.js';
...
export class RedactService {
  ...
  public redactHeaders(
    ...
    for (const [k, v] of Object.entries(headers)) {
      if (Array.isArray(v)) {
        out[k] = v.map((item) => this.redactHeaderValue(k, item)) as string[];
      } else {
   ...
   */
  public deepRedactJson(obj: JsonValue, depth = 0): JsonValue {
    if (depth > 32) return '[MAX_DEPTH]';
    if (obj === null || obj === undefined) return obj;

    if (Array.isArray(obj)) {
      return obj.map((item) => this.deepRedactJson(item, depth + 1));
    }
   ...
   */
  public tryParseJson(buffer: Buffer | null): JsonValue | null {
   ...
   */
  private redactHeaderValue(name: string, value: string | undefined): string | undefined {
   ...
   */
  private redactJsonValue(key: string, value: JsonValue): JsonValue {
