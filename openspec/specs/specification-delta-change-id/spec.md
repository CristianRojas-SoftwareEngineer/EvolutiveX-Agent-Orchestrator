...
3. Liste los directorios bajo `<changesDir>/archive/*/phases/` cuyo nombre empiece por `YYYY-MM-DD--`.
4. Para cada nombre de directorio (raíz o fase), normalice descontando el prefijo `YYYY-MM-DD--` (regex `^\d{4}-\d{2}-\d{2}--`).
5. Extraiga el entero del prefijo `c` con regex `^c(\d+)` sobre el nombre normalizado.
6. Tome el máximo (0 si ninguno coincide) y devuelva `c` + entero incrementado con relleno de ceros a 5 dígitos.
...
#### Scenario: Fase L2 archivada cuenta para el máximo

- **GIVEN** `openspec/changes/archive/2026-06-01--c00012-gateway-migration/phases/2026-05-29--c00008-gateway-g4-audit-projection/` existe
- **AND** el máximo en raíz es `c00069`
- **WHEN** se invoca `computeNextChangeId`
- **THEN** el resultado es `c00070`
...
El módulo SHALL exponer `findDuplicateChangeIds(changesDir)` que devuelve grupos de directorios que comparten el mismo entero `c<NNNNN>` (tras normalizar fecha), incluyendo colisiones entre raíz y fases. `npm run openspec:verify-change-id -- --change <name>` SHALL fallar con código 1 si el entero de `<name>` aparece en más de un directorio.
...
### Requirement: Formato de nombre archivado

Los directorios bajo `openspec/changes/archive/` (raíz y fases L2 bajo `phases/`) SHALL usar el formato `YYYY-MM-DD--c<NNNNN>-<slug>` donde `YYYY-MM-DD` es la fecha de archivado del segmento y `--` es el separador canónico entre fecha e identificador.

#### Scenario: Archivado raíz

- **WHEN** `archive-specification-delta` mueve `c00070-my-feature` el 2026-06-20
- **THEN** el directorio destino es `archive/2026-06-20--c00070-my-feature/`

#### Scenario: Fase L2 anidada en close-out de roadmap

- **WHEN** un L2 `c00008-gateway-g4-audit-projection` se anida bajo el L1 archivado
- **THEN** su ruta es `phases/2026-05-29--c00008-gateway-g4-audit-projection/` (fecha y `c<NNNNN>` del delta archivado)
