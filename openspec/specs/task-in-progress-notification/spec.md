...
4. Si el filtro pasa → invoque el CLI de notificaciones con `DesktopNotificationAdapter` directamente (paridad estructural con `pre-tool-use-hook-ux.ts`), llamando a `buildEvent` + `adapter.notify()`.
