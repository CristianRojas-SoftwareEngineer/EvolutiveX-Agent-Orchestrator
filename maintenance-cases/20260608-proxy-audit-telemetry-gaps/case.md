...
status: done
verdict: especificado — causa SH-α confirmada; OpenSpec fix-proxy-audit-telemetry-gaps apply-ready; implementación pendiente
---

# Case Manifest — 20260608-proxy-audit-telemetry-gaps

## Case

Brechas de telemetría y metadatos en la auditoría causal del Smart Code Proxy detectadas al comparar la sesión `dcdf0a15-4f0b-4a77-864e-1e481b07315c` (turno agentic lineal con 6 tools, sin subagentes) contra el harness nativo de Claude Code: `stepCount` subreportado en `IWorkflowResult`, eventos `tool_result` duplicados en `events.ndjson`, `finalText` duplicado entre workflow sesión y workflow wire, e `interactionType: "main"` fuera de la taxonomía documentada.

## Profile parameters

- **Objetivo:** restaurar coherencia entre proyección en disco, EventBus y metadatos de cierre sin regresiones.
- **Prioridades:** reproduce → root cause → minimal fix → no regression.
- **Success metrics:** test red→green; `stepCount` = hops materializados; 1 `tool_result` por tool; `finalText` en un solo workflow; taxonomía `interactionType` normalizada.
...
openspec_change: "fix-proxy-audit-telemetry-gaps"
case_run: 1
case_paused_at: ""
case_resumed_at: ""
