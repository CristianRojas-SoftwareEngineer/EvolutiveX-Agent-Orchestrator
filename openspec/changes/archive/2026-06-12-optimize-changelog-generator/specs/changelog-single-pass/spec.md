...
- **THEN** se invoca `git log` exactamente una vez; el script detecta los límites de cada release a partir de los refs incluidos en la salida de esa única invocación
...
- **WHEN** la salida de `git log` incluye un subject con prefijo `feat:`, `feat(<scope>):`, `fix:`, `fix(<scope>):`, `perf:`, `refactor:`, `docs:` (con o sin scope, con o without `!`)
...
- **WHEN** se invoca el script con `--pending "msg"` o `--case X`
- **THEN** el script imprime un mensaje de error indicando que el argumento fue eliminado y termina con exit code 1
