...
import { ProxyEnvironmentConfig } from '../src/interfaces/config.interface.js';
...
describe('SessionService - Resolución de sesión', () => {
  it('debería usar la cabecera primaria (override) si está presente', () => {
    const svc = new SessionService(makeConfig(), 'sessions');
  ...
  it('debería deshabilitar el fallback si AUDIT_SESSION_FALLBACK_HEADER=""', () => {
    const svc = new SessionService(
      makeConfig({ AUDIT_SESSION_FALLBACK_HEADER: '' }),
      'sessions',
    );
    const result = svc.getAuditSessionId({
      'x-claude-code-session-id': 'sesion-fb',
  ...
  it('debería añadir sufijo de hash si AUDIT_SESSION_HASH_SUFFIX=true', () => {
    const svc = new SessionService(
      makeConfig({ AUDIT_SESSION_HASH_SUFFIX: true }),
      'sessions',
    );
    const result = svc.getAuditSessionId({
      'x-cc-audit-session': 'mi-sesion',
...
describe('SessionService - Formato de directorio', () => {
