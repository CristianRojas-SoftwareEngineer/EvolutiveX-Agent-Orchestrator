...
**Contraste:** la auditoría del [proxy de observabilidad](../README.md) bajo `sessions/<session-id>/main-agent/interactions/NN/` y `side-interactions/NN/` describe el tráfico hacia **api.anthropic.com**, no un contrato oficial de OpenRouter. Para el árbol de archivos y `meta.json`, véase [`session-audit-model.md`](./session-audit-model.md); la skill opcional **`smart-code-proxy`** (`~/.claude/skills/smart-code-proxy/`) aplica al proxy Anthropic, no a OpenRouter.
