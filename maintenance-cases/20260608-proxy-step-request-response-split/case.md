...
status: done
verdict: resuelto — enrichOpenWireStepWithResponse unifica request/response en un IStep por hop
---

# Case Manifest — 20260608-proxy-step-request-response-split

## Case

Desalineación entre el diseño documentado en `docs/session-audit-model.md` (un step = una carpeta `steps/MM/` con subdirectorios `request/` y `response/`) y la implementación actual, que registra dos `IStep` distintos por hop HTTP (uno en ingress vía `registerWireStepRequest`, otro en egress vía `registerWireStepInCorrelator`), produciendo el doble de carpetas bajo `steps/` y un `stepCount` que no coincide con el conteo de directorios. Evidencia: sesión `d0cce210-92e4-4f3a-b838-15716ca5dd05`, workflow `02` con 6 carpetas `steps/00..05` pero `result.json` con `stepCount: 3`.

## Profile parameters

- **Priorities:** reproduce → root cause → minimal fix → no regression.
- **Success metrics:** test red→green; un hop HTTP → un directorio `steps/MM/` con `request/` y `response/`; `stepCount` coherente con carpetas cerradas; cero regresiones.
...
openspec_change: "2026-06-09-align-wire-step-request-response"
case_run: 1
case_paused_at: ""
case_resumed_at: ""
