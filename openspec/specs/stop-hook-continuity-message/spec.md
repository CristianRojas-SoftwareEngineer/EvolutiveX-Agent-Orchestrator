...
Define el contrato de generación, persistencia y notificación del mensaje de continuidad que el hook `Stop` de Claude Code produce al cierre de cada turno en el proyecto Smart Code Proxy. El mensaje cubre tres dimensiones (qué se completó, qué está abierto, dirección del siguiente prompt) extraídas del contexto del workflow, y se persiste en disco como punto de integración para el sistema TTS (Fase 2, fuera del scope).
...
El sistema SHALL implementar `generateContinuityMessage(context: WorkflowContext): Promise<string | undefined>` en `scripting/stop-work-summary-notification.ts`. Esta función reemplaza `summarizeWorkWithModel` para el hook `Stop`.
...
Este archivo actúa como punto de integración para el sistema TTS (Fase 2, fuera del scope de este spec): TTS leerá el texto completo desde esta ruta fija sin necesidad de parámetros adicionales.
...
2. Si no hay texto generado pero hay texto fuente (`last_assistant_message` o transcript) → `fallbackSummary(assistantText)` (texto normalizado).
...
1. `extractWorkflowContext(transcript_path)` — extrae el contexto del workflow.
2. `generateContinuityMessage(context)` — genera el mensaje de continuidad.
3. `writeContinuityMessage(text, projectDir)` — persiste en disco (no bloquea si falla).
4. Emitir el único toast con el cuerpo resuelto según la jerarquía anterior.
