...
El sistema SHALL definir el tipo `NotificationEvent` en `src/2-services/notifications/types.ts` con exactamente cuatro campos requeridos/opcionales base (`title: string` requerido, `message: string` requerido, `sound?: boolean` opcional con default `false`, `silent?: boolean` opcional con default `false`) más dos campos opcionales de branding: `appId?: string` (identificador de aplicación, AUMID en Windows) e `icon?: string` (ruta a un asset de imagen usado como icono cosmético de la notificación). El resto de campos de personalización SHALL seguir excluido (`image`, `contentImage`, `appIdPath`, `subtitle`, `category`, `urgency`, `timeout`, `wait`, `open`, `closeLabel`, `actions`, `heroImage`).
...
El directorio SHALL contener EXACTAMENTE: `INotificationService.ts`, `DesktopNotificationAdapter.ts`, `types.ts`, `index.ts`, `cli.ts`, `register.ts`, `snoretoast-shortcut.ts`, `lnk-format.ts`, `registry.ts`, `asset-paths.ts`.

El icono cosmético SHALL vivir en `assets/notifications/ai-assistant.png` y `assets/notifications/ai-assistant.ico`, versionados fuera de `src/`. El CLI y `register.ts` los resuelven por convención de path (`<repo-root>/assets/notifications/ai-assistant.{png,ico}`).
...
- **THEN** SHALL existir exactamente: `INotificationService.ts`, `DesktopNotificationAdapter.ts`, `types.ts`, `index.ts`, `cli.ts`, `register.ts`, `snoretoast-shortcut.ts`, `lnk-format.ts`, `registry.ts`, `asset-paths.ts`
...
Cuando `--app-id` no se proporcione, el CLI SHALL aplicar el default `AIAssistant.Proxy` (constante hardcodeada que sigue la convención Windows AUMID: `[Compañía].[App]`, sin espacios, ≤ 129 caracteres). Cuando `--icon` no se proporcione, el CLI SHALL aplicar el default `<repo-root>/assets/notifications/ai-assistant.png`, resuelto al cargar el CLI mediante `path.resolve` desde el `import.meta.url` del módulo (`path.resolve(path.resolve(fileURLToPath(import.meta.url), '..'), '../../..', 'assets/notifications/ai-assistant.png')`; el `resolvePath(..., '..')` interno aplica `dirname` para que `../../..` cuente desde el directorio `src/2-services/notifications/` hasta la raíz del repo — usar el `import.meta.url` sin `dirname` cancelaría primero el nombre de archivo y daría un nivel de más). El `icon` por default SHALL existir como archivo en disco al momento de cargar el CLI; si no existe, el CLI SHALL omitir el campo `icon` del evento y continuar con `appId` por default (degradación con gracia: la notificación se sigue emitiendo).
...
- **THEN** SHALL invocarse `DesktopNotificationAdapter.notify` con un evento que contiene `title: 'UserPromptSubmit'`, `message: 'Hola'`, `appId: 'AIAssistant.Proxy'` y `icon: <ruta absoluta resuelta al .png>`
...
**Directorio estable ASCII-only para assets:** la ruta del repo (`<repo-root>/assets/notifications/ai-assistant.ico`) puede contener caracteres no-ASCII (por ejemplo, la "ó" de "Proyectos" en la ruta del repo actual), lo que causa que las Windows shell APIs (que consultan el icono del AUMID) muestren un icono genérico en lugar del logo AI Assistant. El helper SHALL copiar los assets desde el repo a `%LOCALAPPDATA%\AIAssistant\` (ruta ASCII-only, definida en `asset-paths.ts` como `STABLE_ICON_PATH` y `STABLE_PNG_PATH`) durante `--install`, y SHALL usar esas rutas estables en el registro y en la CLI (toast body image). La copia SHALL ser idempotente (basada en hash SHA-256: si el destino ya coincide con el fuente, no se reescribe). El repo sigue siendo la fuente de verdad de los binarios (`assets/notifications/*.ico|png`); `%LOCALAPPDATA%\AIAssistant\` es un cache operativo.
