...
import { type ProviderTestResult } from './modules/types.js';
import { EXCLUDED_PROVIDERS, resolveTestProviders } from './modules/providers.js';
import {
  ...
  inferMessageType,
} from './modules/log-analyzer.js';
import { announceProviderEnd, announceProviderStart } from './modules/local-announce.js';
import { waitForGatewayTtsDrain } from './modules/wait-for-tts.js';
import {
  killProcessOnPort,
  startProxy,
  stopProxy,
  ...
  sleep,
  getLogPath,
} from './session-lib/proxy-lifecycle.js';
import { getProxyPort, getLogByteOffset } from './session-lib/env-utils.js';
import { buildIsolatedProviderEnv } from './session-lib/provider-env.js';
import { runClaudeHeadless } from './session-lib/run-claude.js';
import {
  findLatestTranscriptPath,
  verifyPromptInTranscript,
  extractLastUserPrompt,
} from './modules/verify-prompt.js';
...
async function testProvider(provider: string, opts: CliOptions): Promise<ProviderTestResult> {
  ...
  try {
    ...
    const messageType = inferMessageType(
      logAnalysis.ttsStatus,
      logAnalysis.ttsFallbacks,
      logAnalysis.ttsSpeeches,
    );
...
function buildResult(
  ...
  const { mainSessionStatus, ttsStatuses, ttsSpeeches, ttsFallbacks, stopUsedFallback, has402 } =
    logAnalysis;
  ...
  return {
    ...
    ttsCallCount,
    anyTtsFallback:
      filterActionableTtsFallbacks(ttsFallbacks).length > 0 || hasAnyTtsHttpFailure(ttsStatuses),
    stopUsedFallback,
    ttsFallbacks,
    ttsWorked,
...
async function testFallbackScenario(opts: CliOptions): Promise<boolean> {
  ...
  try {
    ...
    console.log(
      chalk.gray(
        `  Drenaje TTS: ${drain.ttsCount} llamada(s), ${Math.round(drain.drainMs / 1000)}s`,
      ),
    );
...
function printSuitePlan(providers: string[], excluded: string[], explicitList: boolean): void {
  console.log(chalk.cyan('\n=== Suite headless TTS ==='));
